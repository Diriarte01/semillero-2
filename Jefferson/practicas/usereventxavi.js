...
define(["N/https"], function (https) {
  ...
  function beforeSubmit(context) {
    ...
    if (tipo == "customrecord_s4_categories_exuserevent") {
      let valueId;
    ...
    if (tipo == "customrecord_s4_products_exerciseeu") {
      let valueId;
      ...
      const valueCategoriId = obj.getValue("custrecord_idcatedori_products");
      ...
      if (context.type == context.UserEventType.EDIT) {
        ...
        const valueOldCategoriId = oldObj.getValue(
          "custrecord_idcatedori_products"
        );
        const valueOldDescription = oldObj.getValue(
          "custrecord_description_prodructs"
        );
        const valueOldImage = oldObj.getValue("custrecordimage");
        if (valueImage != valueOldImage) {
          object.image = valueOldImage;
    ...
    if (tipo == "customrecord_s4_users_exerciseeu") {
      let valueId;
